...
static void main_loop(void *pvParameters)
{
  ...
  for (;;)
  {
    ...
    if (ret != 0)
      printf("Reading IMU raw failed with code %d\n", ret);
    ...
    else if (my_state == RUNNING || my_state == WOUND_UP)
    {
          ...
          abs(sin_roll) < ROLL_LIMIT)
      {
        ...
        {
          my_state = WOUND_UP;
   //       printf("Setting eyes to blue - wound up\n");
          set_both_eyes(BLUE);
        }
       ...
       // printf("sin_pitch target_angle motor_speed motor_bias %d, %d, %d, %d\n", sin_pitch, (target_angle - FLT_TO_Q16(HIGH_PID_LIMIT), motor_speed, motor_bias);
      }
      else
      {
        my_state = FALLEN;
    //    printf("Setting eyes to blue - fallen\n");
        set_both_eyes(BLUE);
        travel_speed = 0;
    ...
    {
      printf("%d, %d\n", sin_pitch - STABLE_ANGLE, sin_roll);
    }
...
extern "C" void user_init(void)
{
  ...
  imu_i2c_configure(I2C_FREQ_800K, IMU_SCL_PIN, IMU_SDA_PIN);
  int ret = imu_init();
  if (ret != 0)
    printf("IMU init failed with code %d\n", ret);
  eyes_init();
    printf("Setting eyes to yellow or red\n");
